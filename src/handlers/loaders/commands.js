...
module.exports = (client) => {
    ...
    if (!client.shard || client.shard.ids[0] === 0) console.log(chalk.blue(chalk.bold(`System`)), (chalk.white(`>>`)), (chalk.green(`Loading commands`)), (chalk.white(`...`)))
    if (!client.shard || client.shard.ids[0] === 0) console.log(`\u001b[0m`);
    ...
    fs.readdirSync('./src/interactions').forEach(dirs => {
        ...
        if (!client.shard || client.shard.ids[0] === 0) console.log(chalk.blue(chalk.bold(`System`)), (chalk.white(`>>`)), chalk.red(`${commandFiles.length}`), (chalk.green(`commands of`)), chalk.red(`${dirs}`), (chalk.green(`loaded`)));
